...
class Katava extends StatefulWidget {
  const Katava({Key? key}) : super(key: key);

  @override
  _KatavaState createState() => _KatavaState();
}

class _KatavaState extends State<Katava> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  KatavaData data = KatavaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.apotekice2.length; i++) {
      double storelat = data.apotekice2[i]['lat'];
      double storelng = data.apotekice2[i]['lng'];
      ...
      data.apotekice2[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.17076832181201, 17.65833221628702),
  ];
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.apotekice2[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.apotekice2[0]['distance'].round()} m away"
                                          : "${data.apotekice2[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
