...
class Lasva extends StatefulWidget {
  const Lasva({Key? key}) : super(key: key);

  @override
  _LasvaState createState() => _LasvaState();
}

class _LasvaState extends State<Lasva> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  LasvaData data = LasvaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.lasvaci.length; i++) {
      double storelat = data.lasvaci[i]['lat'];
      double storelng = data.lasvaci[i]['lng'];
      ...
      data.lasvaci[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.174156003812286, 17.66238590900189),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/samoposluga/lasva.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.lasvaci[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.lasvaci[0]['distance'].round()} m away"
                                          : "${data.lasvaci[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: <Widget>[
                                ...
                                Text(
                                  (BHS == true)
                                      ? 'Radno vrijeme: 07:30/21'
                                      : (English == true)
                                          ? "Working hours: 07:30/21"
                                          : "Arbeitszeit: 07:30/21",
