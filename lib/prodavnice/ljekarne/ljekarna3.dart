...
class Ljekarna3 extends StatefulWidget {
  const Ljekarna3({Key? key}) : super(key: key);

  @override
  _LjekarnaState3 createState() => _LjekarnaState3();
}

class _LjekarnaState3 extends State<Ljekarna3> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  LjekarnaApotekaData3 data = LjekarnaApotekaData3();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.apotekice3.length; i++) {
      double storelat = data.apotekice3[i]['lat'];
      double storelng = data.apotekice3[i]['lng'];
      ...
      data.apotekice3[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.17090273375481, 17.65865198391307),
  ];
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.apotekice3[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.apotekice3[0]['distance'].round()} m away"
                                          : "${data.apotekice3[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
