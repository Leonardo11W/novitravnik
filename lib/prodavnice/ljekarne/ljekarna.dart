...
import 'package:novi_travnik/prodavnice/ljekarne/ljekarneposebno.dart';
import 'package:novi_travnik/search.dart';

import 'package:geolocator/geolocator.dart';

class Ljekarna extends StatefulWidget {
  const Ljekarna({Key? key}) : super(key: key);

  @override
  _LjekarnaState createState() => _LjekarnaState();
}

class _LjekarnaState extends State<Ljekarna> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  LjekarnaApotekaData data = LjekarnaApotekaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.apotekice.length; i++) {
      double storelat = data.apotekice[i]['lat'];
      double storelng = data.apotekice[i]['lng'];
      ...
      data.apotekice[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.1705858361591, 17.658593008860393),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/samoposluga/apoteka.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.apotekice[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.apotekice[0]['distance'].round()} m away"
                                          : "${data.apotekice[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
                          ...
                          Padding(
                            ...
                            child: Row(
                              children: <Widget>[
                                ...
                                Text(
                                  (BHS == true)
                                      ? 'Radno vrijeme: 07:30/15'
                                      : (English == true)
                                          ? "Working hours: 07:30/15"
                                          : "Arbeitszeit: 07:30/15",
