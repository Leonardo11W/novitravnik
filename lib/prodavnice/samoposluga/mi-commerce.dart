...
class Mikomerc extends StatefulWidget {
  const Mikomerc({Key? key}) : super(key: key);

  @override
  _MikomercState createState() => _MikomercState();
}

class _MikomercState extends State<Mikomerc> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  MiKomercData data = MiKomercData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.mikomerci.length; i++) {
      double storelat = data.mikomerci[i]['lat'];
      double storelng = data.mikomerci[i]['lng'];
      ...
      data.mikomerci[i]['distance'] = (distance!);
      setState(() {});
    }
  }
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/samoposluga/mikomerc.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.mikomerci[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.mikomerci[0]['distance'].round()} m away"
                                          : "${data.mikomerci[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
                          ...
                          Padding(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  Text(
                                    (BHS == true)
                                        ? 'Adresa: 28 Ljudevita Gaja'
                                        : (English == true)
                                            ? "Address: 28 Ljudevita Gaja"
                                            : "die Anschrift: 28 Ljudevita Gajas",
                                    textAlign: TextAlign.left,
                                    style: TextStyle(
                                        color: (Click == false)
                                            ? Colors.white
                          ...
                          Padding(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  RichText(
                                      text: new TextSpan(
                                          text: 'mi-commerce.business.site',
                                          style: TextStyle(
                                              color: (Click == false)
                                                  ? Colors.white
                                                  : Colors.black),
                                          recognizer: new TapGestureRecognizer()
                                            ..onTap = () async {
                                              launch(
                                                  'https://mi-commerce.business.site/');
                                            }))
                                ],
                              )),
                        ],
