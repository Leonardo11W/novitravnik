...
import 'package:novi_travnik/ugostiteljstvo/kafici/posebnokafici.dart';

import 'package:url_launcher/url_launcher.dart';

class Caramela extends StatefulWidget {
  const Caramela({Key? key}) : super(key: key);

  @override
  _CaramelaState createState() => _CaramelaState();
}

class _CaramelaState extends State<Caramela> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  CaramelaData data = CaramelaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.caramelici.length; i++) {
      double storelat = data.caramelici[i]['lat'];
      double storelng = data.caramelici[i]['lng'];
      ...
      data.caramelici[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.16887060504823, 17.649394162502333),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/kafici/caramela.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  "${data.caramelici[0]['distance'].round()} m udaljeno",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
                          ...
                          Padding(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  Text(
                                    (BHS == true)
                                        ? 'Adresa: Patriotske lige 3'
                                        : (English == true)
                                            ? "Address: Patriotske lige 3"
                                            : "die Anschrift: Patriotske lige 3",
                                    textAlign: TextAlign.left,
                                    style: TextStyle(
                                        color: (Click == false)
                                            ? Colors.white
                          ...
                          Padding(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  RichText(
                                      text: new TextSpan(
                                          text: 'caffe-caramela.business.site',
                                          style: TextStyle(
                                              color: (Click == false)
                                                  ? Colors.white
                                                  : Colors.black),
                                          recognizer: new TapGestureRecognizer()
                                            ..onTap = () async {
                                              launch(
                                                  'https://caffe-caramela.business.site/?utm_source=gmb&utm_medium=referral');
                                            }))
                                ],
                              )),
                        ],
