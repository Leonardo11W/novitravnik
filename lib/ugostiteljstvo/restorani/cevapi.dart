...
import 'package:novi_travnik/ugostiteljstvo/restorani/cevapidata.dart';
...
class Cevapi extends StatefulWidget {
  const Cevapi({Key? key}) : super(key: key);

  @override
  _CevapiState createState() => _CevapiState();
}

class _CevapiState extends State<Cevapi> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  CevapiData data = CevapiData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.cevapi.length; i++) {
      double storelat = data.cevapi[i]['lat'];
      double storelng = data.cevapi[i]['lng'];
      ...
      data.cevapi[i]['distance'] = (distance!);
      setState(() {});
    }
  }
                      ...
                      children: [
                        Container(
                          ...
                          child: CachedNetworkImage(
                            imageUrl: data.cevapi[index]['image'],
                            fit: BoxFit.fill,
                          ),
                        ),
                        SizedBox(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              (BHS == true)
                                  ? "${data.cevapi[index]['distance'].round()} m udaljeno"
                                  : (English == true)
                                      ? "${data.cevapi[index]['distance'].round()} m away"
                                      : "${data.cevapi[index]['distance'].round()} m weg",
                              style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                              ),
