...
class Venezia extends StatefulWidget {
  const Venezia({Key? key}) : super(key: key);

  @override
  _VeneziaState createState() => _VeneziaState();
}

class _VeneziaState extends State<Venezia> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  VeneziaData data = VeneziaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.venezi.length; i++) {
      double storelat = data.venezi[i]['lat'];
      double storelng = data.venezi[i]['lng'];
      ...
      data.venezi[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.17427771005521, 17.663444627639215),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/venezia.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.venezi[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.venezi[0]['distance'].round()} m away"
                                          : "${data.venezi[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
