...
class Zeko extends StatefulWidget {
  const Zeko({Key? key}) : super(key: key);

  @override
  _ZekoState createState() => _ZekoState();
}

class _ZekoState extends State<Zeko> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  ZekoData data = ZekoData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.zeki.length; i++) {
      double storelat = data.zeki[i]['lat'];
      double storelng = data.zeki[i]['lng'];
  ...
  final List<LatLng> _latLang = [
    LatLng(44.168638668197296, 17.64733618086392),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/zeko.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.zeki[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.zeki[0]['distance'].round()} m away"
                                          : "${data.zeki[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
