...
class Milano extends StatefulWidget {
  const Milano({Key? key}) : super(key: key);

  @override
  _MilanoState createState() => _MilanoState();
}

class _MilanoState extends State<Milano> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  MilanoData data = MilanoData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.milini.length; i++) {
      double storelat = data.milini[i]['lat'];
      double storelng = data.milini[i]['lng'];
      ...
      data.milini[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.17478830845236, 17.66341628530757),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/milano.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.milini[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.milini[0]['distance'].round()} m away"
                                          : "${data.milini[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
