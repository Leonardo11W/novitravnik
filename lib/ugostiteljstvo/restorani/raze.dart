...
class _RezaState extends State<Raze> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  RazeData data = RazeData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.razi.length; i++) {
      double storelat = data.razi[i]['lat'];
      double storelng = data.razi[i]['lng'];
  ...
  final List<LatLng> _latLang = [
    LatLng(44.16893728907705, 17.653206646028536),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/reza.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.razi[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.razi[0]['distance'].round()} m away"
                                          : "${data.razi[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
