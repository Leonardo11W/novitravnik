...
import 'package:novi_travnik/ugostiteljstvo/restorani/cevapiposebno.dart';

import 'package:novi_travnik/ugostiteljstvo/ugostiteljstvo.dart';

import 'package:url_launcher/url_launcher.dart';

class Neslanovic extends StatefulWidget {
  const Neslanovic({Key? key}) : super(key: key);

  @override
  _NeslanovicState createState() => _NeslanovicState();
}

class _NeslanovicState extends State<Neslanovic> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  NeslanovicData data = NeslanovicData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.neslanovici.length; i++) {
      double storelat = data.neslanovici[i]['lat'];
      double storelng = data.neslanovici[i]['lng'];
      ...
      data.neslanovici[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.168782962568045, 17.653427477528364),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/neslanovic.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.neslanovici[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.neslanovici[0]['distance'].round()} m away"
                                          : "${data.neslanovici[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
