...
class Mima extends StatefulWidget {
  const Mima({Key? key}) : super(key: key);

  @override
  _MimaState createState() => _MimaState();
}

class _MimaState extends State<Mima> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  MimaData data = MimaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.mimici.length; i++) {
      double storelat = data.mimici[i]['lat'];
      double storelng = data.mimici[i]['lng'];
      ...
      data.mimici[i]['distance'] = (distance!);
      setState(() {});
    }
  }
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/mima.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.mimici[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.mimici[0]['distance'].round()} m away"
                                          : "${data.mimici[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
