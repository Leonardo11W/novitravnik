...
import 'package:novi_travnik/ugostiteljstvo/posebnoslasticarne.dart';
import 'package:novi_travnik/ugostiteljstvo/slasticarne.dart';
import 'package:url_launcher/url_launcher.dart';

class Granita extends StatefulWidget {
  const Granita({Key? key}) : super(key: key);

  @override
  _GranitaState createState() => _GranitaState();
}

class _GranitaState extends State<Granita> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  GranitaData data = GranitaData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.graniti.length; i++) {
      double storelat = data.graniti[i]['lat'];
      double storelng = data.graniti[i]['lng'];
      ...
      data.graniti[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.169356489506065, 17.655803072257413),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/granita.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.graniti[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.graniti[0]['distance'].round()} m away"
                                          : "${data.graniti[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
