...
import 'package:novi_travnik/ugostiteljstvo/lounge/loungedata.dart';
...
class _LoungeState extends State<Lounge> {
  ...
  LoungeData data = LoungeData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.lounges.length; i++) {
      double storelat = data.lounges[i]['lat'];
      double storelng = data.lounges[i]['lng'];
      ...
      data.lounges[i]['distance'] = (distance!);
      setState(() {});
    }
  }
                      ...
                      children: [
                        Container(
                          ...
                          child: CachedNetworkImage(
                            imageUrl: data.lounges[index]['image'],
                            fit: BoxFit.fill,
                          ),
                        ),
                        SizedBox(
                        ...
                        Row(
                          ...
                          children: [
                            ...
                            Text(
                              (BHS == true)
                                  ? "${data.lounges[index]['distance'].round()} m udaljeno"
                                  : (English == true)
                                      ? "${data.lounges[index]['distance'].round()} m away"
                                      : "${data.lounges[index]['distance'].round()} m weg",
                              style: TextStyle(
                                fontSize: 18,
                                color: Colors.white,
                              ),
