...
class Kokos extends StatefulWidget {
  const Kokos({Key? key}) : super(key: key);

  @override
  _KokosState createState() => _KokosState();
}

class _KokosState extends State<Kokos> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  KokosData data = KokosData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.kokosi.length; i++) {
      double storelat = data.kokosi[i]['lat'];
      double storelng = data.kokosi[i]['lng'];
      ...
      data.kokosi[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.17386351613208, 17.66295315120107),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/restoran/kokos.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.kokosi[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.kokosi[0]['distance'].round()} m away"
                                          : "${data.kokosi[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
