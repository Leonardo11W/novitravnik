...
import 'package:novi_travnik/ostalo/sport/teretane/teretanespecial.dart';
...
class Spartan extends StatefulWidget {
  const Spartan({Key? key}) : super(key: key);

  @override
  _SpartanState createState() => _SpartanState();
}

class _SpartanState extends State<Spartan> {
  Position? _currentUserPosition;
  double? distanceImMeter = 0.0;
  LuganGymData data = LuganGymData();
  Future _getTheDistance() async {
    _currentUserPosition = await Geolocator.getCurrentPosition(
        desiredAccuracy: LocationAccuracy.high);

    for (int i = 0; i < data.lionici.length; i++) {
      double storelat = data.lionici[i]['lat'];
      double storelng = data.lionici[i]['lng'];
      ...
      data.lionici[i]['distance'] = (distance!);
      setState(() {});
    }
  }
  ...
  final List<LatLng> _latLang = [
    LatLng(44.1712877152381, 17.66255800781075),
  ];
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          CachedNetworkImage(
                            imageUrl:
                                'https://photos.novitravnik.online/jos/spartanfitness.png',
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Text(
                                  (BHS == true)
                                      ? "${data.lionici[0]['distance'].round()} m udaljeno"
                                      : (English == true)
                                          ? "${data.lionici[0]['distance'].round()} m away"
                                          : "${data.lionici[0]['distance'].round()} m weg",
                                  maxLines: 1,
                                  overflow: TextOverflow.fade,
                                  softWrap: false,
                                  textAlign: TextAlign.center,
                          ...
                          Padding(
                              ...
                              child: Row(
                                children: <Widget>[
                                  ...
                                  Text(
                                    (BHS == true)
                                        ? 'Adresa: Ruđera Boškovića 37a'
                                        : (English == true)
                                            ? "Address: Ruđera Boškovića 37a"
                                            : "die Anschrift: Ruđera Boškovića 37a",
                                    textAlign: TextAlign.left,
                                    style: TextStyle(
                                        color: (Click == false)
                                            ? Colors.white
